...
tcp_server_t* tcpserver_create(char* server_name, char* log_filename, int max_clients, int connection_queue_size, int port, bool listen_console){
	...
	server->logger = log_create(log_filename, server_name, false, LOG_LEVEL_INFO);
...
void tcpserver_destroy(tcp_server_t* server){
	...
	log_destroy(server->logger);

	free(server);
}

bool tcpserver_accept_new_connection(tcp_server_t* server, void (*on_accept)(tcp_server_t*, int)){
	int new_socket;
