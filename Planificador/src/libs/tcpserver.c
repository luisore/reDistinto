...
tcp_server_t* tcpserver_create(char* server_name, t_log* log, int max_clients,
	...
	log_info(server->logger, "Initializing TCP server: %s on port %d.",
			server_name, port);
	...
	if ((server->master_socket = socket(AF_INET, SOCK_STREAM, 0)) == 0) {
		log_error(server->logger,
				"Could not create master socket for server: %s.", server_name);
	...
	if (setsockopt(server->master_socket, SOL_SOCKET, SO_REUSEADDR,
		...
		log_error(server->logger,
				"Could not configure server %s for multiple connections on master socket.",
				server_name);
		tcpserver_destroy(server);
		return NULL;
	}
	log_info(server->logger, "TCP Server %s created on socket: %d", server_name,
	...
	if (bind(server->master_socket, (struct sockaddr *) (server->address),
		...
		log_error(server->logger, "Could not bind TCP Server %s to port %d.",
				port, server_name);
		tcpserver_destroy(server);
		return NULL;
	}
	log_info(server->logger, "TCP Server %s bound to port: %d.", server_name,
			port);
...
bool tcpserver_accept_new_connection(tcp_server_t* server,
		void (*on_accept)(tcp_server_t*, int, int)) {
	int new_socket;
	int addrlen = sizeof(server->address);

	if ((new_socket = accept(server->master_socket,
			(struct sockaddr *) (server->address), (socklen_t*) &addrlen))
			< 0) {
		log_error(server->logger,
				"Could not accept connection on socket for server: %s",
				server->name);
		return false;
	}
	...
	log_info(server->logger,
			"New connection received on TCP Server: %s. Socket: %d, IP:: %s, Port: %d.",
			server->name, new_socket, inet_ntoa(server->address->sin_addr),
	...
	for (i = 0; i < server->max_clients; i++) {
		...
		if (server->client_sockets[i] == 0) {
			...
			log_info(server->logger,
					"TCPServer: %s. Added new connection to list of sockets as: %d.",
					server->name, i);
	...
	log_info(server->logger,
			"Connection on TCP Server: %s received successfully.",
			server->name);
...
int tcpserver_run(tcp_server_t* server, void (*before_cycle)(tcp_server_t*),
	...
	log_info(server->logger, "TCP Server %s waiting for connections ...",
			server->name);
	...
	while (true) {
		...
		if ((activity < 0) && (errno != EINTR)) {
			log_error(server->logger, "Error on select for server: %s",
					server->name);
			return EXIT_FAILURE;
		}
